...
| DEC-P03 | Chu kỳ có 12–14 ngày nền hợp lệ được phép lấp từ mức đại diện chu kỳ | Bật, có log | ĐÃ KHÓA 2026-07-20 (engine đã cài `enableTier2CycleFallback=true` mặc định) |
| DEC-P04 | Chu kỳ có 8–11 ngày nền hợp lệ | Chỉ lấp khi dữ liệu trải ít nhất 2/3 đoạn và chuyển `REVIEW_REQUIRED` | ĐÃ KHÓA 2026-07-20 (engine đã cài, gắn `reviewRequired=true`) |
| DEC-P05 | Chu kỳ có 1–7 ngày nền | Không dùng chính chu kỳ làm nguồn duy nhất | ĐÃ KHÓA 2026-07-20 (engine giữ nguyên hành vi cũ: không lấp) |
...
| DEC-P11 | Chặng 11 (dự báo nền) — cửa sổ lịch sử tối thiểu/khuyến nghị theo từng mô hình, tính ngược từ chu kỳ gần nhất, thay vì luôn dùng toàn bộ chuỗi khóa liên tục (hiện tới ~75 CK/3 năm) | SES: tối thiểu 12 CK, tin cậy 18 CK · Holt: tối thiểu 15 CK, tin cậy 24 CK · Holt-Winters: tối thiểu 2 mùa, tin cậy 3 mùa (48–72 CK) · Croston: tối thiểu 24 CK và đủ điểm phát sinh nhu cầu, tin cậy 48 CK | ĐỀ XUẤT 2026-07-20 (do người dùng cung cấp trực tiếp trong phiên rà soát; đã triển khai trong engine — `SimulationPolicy.forecastWindowCycles`, mặc định bật — nhưng cần BA/Forecast Owner duyệt chính thức trước khi coi là ngưỡng vận hành cuối cùng, đúng RULE mục 5.1 quy trình) |
...
| DEC-R06 | RULE-05-003 / GT-18, GT-19: lấp Tầng 2 theo mức đại diện chu kỳ | 04-Dac-ta §8 mô tả đầy đủ ngưỡng 12-14/8-11/1-7/0; 07-Golden-Test yêu cầu GT-18 "lấp 1 ngày, LOCKED_ADJUSTED", GT-19 "lấp theo cấu hình, ít nhất LOCKED_WITH_REVIEW" | ĐÃ SỬA 2026-07-20 — Tầng 2 đã cài đặt trong `aggregateCycles` (`stage-support.ts`), `enableTier2CycleFallback=true` mặc định, `reviewRequired` mới thêm vào `CycleRecord`. GT-18/GT-19 nay ĐÃ ĐẠT (xem `stage5-cycle-fill.spec.ts`) | ĐÃ KHÓA 2026-07-20 |
| DEC-R08 | RULE-03-002/§Chặng 3 mục 8 (Tài liệu giải pháp): chỉ cận DƯỚI (đầu lịch sử đã đóng) mới khóa vĩnh viễn `KHÔNG CÂN BẰNG CỐ ĐỊNH`; cận TRÊN (gần ngày hiện tại) phải giữ `TẠM · KIỂM TRA` để phiên sau còn xét lại | Tài liệu phân biệt rõ hai hướng biên có hệ quả khác nhau (bảng mục 8) | ĐÃ SỬA 2026-07-20 — `qualifySelection` (`stage-support.ts`) trước đây gộp `nearLowerBoundary !== nearUpperBoundary` nên cận TRÊN cũng bị khóa `fixed` nhầm như cận dưới; nay chỉ `nearLowerBoundary` mới khóa `fixed`, cận trên giữ `temporary` kèm ghi chú. Không đổi giá trị baseDemand/median, chỉ đổi nhãn trạng thái. Dùng chung cho Chặng 3 và Chặng 4 | ĐÃ KHÓA 2026-07-20 |
...
| DEC-R09 | RULE-04-002: ngày CTKM chồng lấn (có giao dịch thật + chỉ suy từ interval) phải gộp đúng cùng một chuỗi mã liên tục; hai cụm CTKM liền kề tuyệt đối (không ngày nào xen giữa) luôn thuộc cùng một giai đoạn liên tục | "Nhóm liên tục theo SKU — nơi bán — loại/chương trình... không làm mất mã" | ĐÃ SỬA 2026-07-20 — (1) `calendar-scaffold.ts`: `promoCode` ghép `[existing?.promoCode, intervalPromo.code]` không khử trùng/sắp xếp nên cùng một cặp mã chồng lấn ra 2 chuỗi khác nhau tùy ngày có giao dịch thật hay không (VD "27949\|27903\|27949" ≠ "27903\|27949"), làm `buildPromoRegions` so chuỗi tuyệt đối cắt gãy giả một cụm CTKM liên tục — nay khử trùng + sắp xếp trước khi ghép. (2) `buildPromoRegions` (`stage-support.ts`): hai cụm liền kề tuyệt đối nhưng đổi mã trước đây chỉ gộp khi một bên tự tính `insufficient` — do `stopAtPromoBoundary` luôn thấy "0 ngày sạch" ngay tại ranh giới đang xét (không phản ánh đúng khả năng thật), khiến cụm không gộp dù liền kề tuyệt đối; nay liền kề tuyệt đối luôn gộp. Phát hiện qua SKU 036 Gừng Gari pink 250g (30/6–31/7/2023), xác nhận bằng dữ liệu thật: cụm nay đạt `balanced` (trước chỉ `temporary`/`before=14 sau=0`) | ĐÃ KHÓA 2026-07-20 |
