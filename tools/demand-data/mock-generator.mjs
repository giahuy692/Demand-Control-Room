...
export function generateDailyRows(definition, runDate, cycleLength, maxCycles) {
  ...
  for (let day = 0; day < totalDays; day++) {
    ...
    rows.push({
      storeCode: 11,
      productCode: Number(definition.id.replace('SKU-', '')),
      barcode: definition.id,
      productName: definition.name,
      date: iso,
      hasSalesRecord: true,
      sales: sold,
      price: definition.price,
      promotionCode: promoCode === 'MEMBER' ? 999 : null,
      promotionName: promoCode === 'MEMBER' ? 'Member Day' : null,
      promotionStartDate: promoCode === 'MEMBER' ? '2026-06-01' : null,
      promotionEndDate: promoCode === 'MEMBER' ? '2026-06-30' : null,
      promotionType: promoCode === 'MEMBER' ? 2 : null,
      promotionMechanismType: promoCode === 'MEMBER' ? 2 : null,
      promotionClass: promoCode === 'MEMBER' ? 'DEEP_PROMO' : 'NO_PROMOTION',
      openStock,
      closeStock,
      receiptHour: receiptHour ? Number(receiptHour.slice(0, 2)) : null,
      stockStatus: openStock < 0 || closeStock < 0 ? 'NEGATIVE_STOCK' : 'CALCULATED',
    });
    stock = closeStock;
  }
  return rows;
