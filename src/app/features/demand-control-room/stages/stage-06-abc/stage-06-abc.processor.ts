import { calendarWindowAbcMetrics } from '../../domain/math';
import { AbcClass, ExceptionTask, SimulationPolicy, StageSnapshot } from '../../domain/models';
...
export function runStage6(previous: StageSnapshot, policy: SimulationPolicy): StageSnapshot {
  ...
  return createSnapshot(6, policy, states, {
    ...
    `[RULE-06-003][RULE-05-006] Cửa sổ ABC là ${windowSize} vị trí chu kỳ gần nhất theo lịch, giữ nguyên mọi vị trí (kể cả chưa khóa) để audit; năm hóa CHỈ dùng đoạn CK khóa LIÊN TIẾP kết thúc tại CK đủ điều kiện gần nhất (không đếm CK khóa rải rác hai phía một khoảng unresolved), tối thiểu ${minimumLockedCycles} CK khóa liên tiếp mới năm hóa. ${fullCoverageCount} SKU FULL_COVERAGE, ${withGapsCount} SKU ANNUALIZED_WITH_GAPS (đoạn liên tiếp ngắn hơn cửa sổ ${windowSize}), ${notRatedCount} SKU NOT_RATED (dưới ${minimumLockedCycles} CK khóa liên tiếp, ABC_INPUT_BLOCKED).`,
  ], exceptions);
}
