...
import { BaseDemandSource, StageNumber, StageSnapshot } from './models';
import { buildStageTrace, shiftLegacyTraceText } from './stage-trace';
...
describe('stage-trace sanity', () => {
  ...
  it('trace chặng 3 thế số đúng bằng kết quả engine đã khóa', () => {
    ...
    for (const state of Object.values(snapshot.states)) {
      const lifted = state.daily.find(record => record.baseDemandSource === BaseDemandSource.STOCKOUT_BASELINE);
      if (!lifted) continue;
  ...
  it('trace chặng 4 dùng median, không dùng max', () => {
    ...
    for (const state of Object.values(snapshot.states)) {
      const normalized = state.daily.find(record => record.baseDemandSource === BaseDemandSource.PROMOTION_BASELINE);
  ...
  it('trace Chặng 7 đủ 8 bước ABC và giải thích a_N', () => {
  ...
  it('trace Chặng 8 đủ các mục 4.4.1–4.4.9 và khớp μ/σ/CV² engine', () => {
    ...
    for (const state of Object.values(snapshots[8]!.states)) {
      if (state.classification.classificationStatus !== 'CLASSIFIED') continue;
      const trace = buildStageTrace(8, state, DEFAULT_POLICY, null);
  ...
  it('process-panel giữ đủ số bước chuẩn của tài liệu cho các chặng có quy trình cố định', () => {
    ...
    const expected: Partial<Record<StageNumber, number>> = {
      1: 10, 2: 4, 3: 8, 4: 8, 6: 8, 7: 8, 8: 9, 9: 8, 13: 7, 14: 6, 15: 5, 16: 10,
      17: 13, 18: 10, 19: 9, 20: 12,
    };
    ...
    const stage9State = Object.values(snapshots[10]!.states).find(state => state.classification.xyz === 'Y' && state.cycles.filter(cycle => cycle.locked).length >= 48)!;
    expect(buildStageTrace(10, stage9State, DEFAULT_POLICY, null).steps).toHaveLength(8);
    const stage10State = Object.values(snapshots[11]!.states).find(state => state.classification.xyz === 'Y' && state.seasonality !== 'confirmed' && state.cycles.filter(cycle => cycle.locked).length >= 12)!;
    expect(buildStageTrace(11, stage10State, DEFAULT_POLICY, null).steps).toHaveLength(8);
    // Chặng 12 có 3 hình dạng khác nhau: X/Y đi qua cửa chu kỳ ngắn 11XY-SN (thêm 1 bước so với Z),
    // Z chạy thẳng Croston/nhịp phát sinh (không có cửa này), D không backtest thống kê [C11 §3, §8.3, §9].
    const stage11XY = Object.values(snapshots[12]!.states).find(state => state.classification.xyz === 'X' || state.classification.xyz === 'Y')!;
    expect(buildStageTrace(12, stage11XY, DEFAULT_POLICY, null).steps, 'Chặng 12 · nhóm X/Y').toHaveLength(10);
    const stage11Z = Object.values(snapshots[12]!.states).find(state => state.classification.xyz === 'Z')!;
    expect(buildStageTrace(12, stage11Z, DEFAULT_POLICY, null).steps, 'Chặng 12 · nhóm Z').toHaveLength(9);
    const stage11D = Object.values(snapshots[12]!.states).find(state => state.classification.xyz === 'D')!;
    expect(buildStageTrace(12, stage11D, DEFAULT_POLICY, null).steps, 'Chặng 12 · nhóm D').toHaveLength(4);
  });

  it('trace B1 Chặng 12 dùng đúng forecast.reason, không tự suy diễn nhãn model rồi lệch với model thực khóa', () => {
    for (const state of Object.values(snapshots[12]!.states)) {
      if (!state.forecast || state.classification.xyz === 'D') continue;
      const trace = buildStageTrace(12, state, DEFAULT_POLICY, null);
      expect(trace.steps[0].detail).toContain(shiftLegacyTraceText(state.forecast.reason));
  ...
  it('trace chặng 15 khớp safety stock engine', () => {
    const snapshot = snapshots[16]!;
    for (const state of Object.values(snapshot.states)) {
      const trace = buildStageTrace(16, state, DEFAULT_POLICY, null);
      const final = trace.steps.at(-1)!;
  ...
  it('panel Chặng 15–20 nêu rõ khi một trường dùng giá trị mặc định vì ERP chưa có cột nguồn, và hiển thị đủ các trường mới đã triển khai', () => {
    const firstState = (stage: StageNumber) => Object.values(snapshots[stage]!.states)[0];
    ...
    // Chặng 15: hàng giữ/hư hỏng/khóa/không bán được nay là trường thật, nhưng ERP chưa có cột nguồn nên vẫn mặc định 0 — panel phải nói rõ điều này, không im lặng suy diễn là đã đủ.
    expect(text(15)).toContain('CHƯA CÓ TRƯỜNG RIÊNG');
    // Chặng 16: mức cần bảo vệ = max(SS, DisplayMin) phải hiển thị rõ cả hai vế.
    expect(text(16)).toContain('DisplayMin');
    // Chặng 17: vùng cần bao phủ (CoverWindow) nay tính theo lead time thật của SKU, không còn cứng toàn bộ tầm dự báo.
    expect(text(17)).toContain('CoverWindow');
    // Chặng 18: giá vốn kế hoạch (landed cost) chưa có cấu hình nguồn nên tạm dùng giá mua và phải cảnh báo là ước tính.
    expect(text(18)).toContain('bucket = CHƯA CẤU HÌNH');
    // Chặng 19: chưa có kênh ghi nhận người dùng tự sửa số đề xuất nên Q_approved_over luôn bằng 0, panel phải nêu rõ lý do thay vì suy diễn là đã kiểm tra đủ.
    expect(text(19)).toContain('Q_approved_over=0');
    // Chặng 20: sai số dự báo NỀN và dự báo CUỐI nay được tách riêng và tính đủ — cả hai nhãn phải xuất hiện.
    expect(text(20)).toContain('WAPE_base');
    expect(text(20)).toContain('WAPE_final');
  });
});
