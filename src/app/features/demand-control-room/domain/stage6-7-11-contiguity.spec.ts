...
/**
 * Kỹ thuật chung: chạy engine thật tới Chặng 6 (dữ liệu giả gapless mặc định), rồi cấy trực tiếp
 ...
 * khoảng đứt quãng theo đúng vị trí mong muốn, sau đó chạy tiếp Chặng 7/7/8/11 trên state đã cấy.
 */
...
function runTo6(): { engine: SimulationEngine; snapshot: StageSnapshot } {
  const engine = testEngine();
  let snapshot: StageSnapshot | null = null;
  for (let stage = 1; stage <= 6; stage++) snapshot = engine.run(stage as StageNumber, snapshot, DEFAULT_POLICY);
...
describe('RULE-07-003/RULE-06-006 — cửa sổ ABC là 24 vị trí gần nhất theo lịch, chỉ năm hóa đoạn CK khóa LIÊN TIẾP (GT-32/GT-34)', () => {
  it('GT-32: CK14, CK15, CK25 khóa nhưng CK16-24 unresolved → không nối thành chuỗi, ABC bị chặn', () => {
    const { engine, snapshot } = runTo6();
    ...
    const s6 = engine.run(7, snapshot, DEFAULT_POLICY);
...
describe('RULE-08-003 — cửa sổ XYZ là đúng 24 vị trí gần nhất theo lịch, có gap trong cửa sổ → CLASSIFICATION_BLOCKED', () => {
  it('GT-23/32: một chu kỳ BASELINE_UNRESOLVED nằm trong cửa sổ 24 vị trí gần nhất → xyz=null, classificationBlockReason=BASELINE_UNRESOLVED, không phải D', () => {
    ...
    const s6 = engine.run(7, snapshot, DEFAULT_POLICY);
    const s7 = engine.run(8, s6, DEFAULT_POLICY);
    ...
    expect(task).toBeDefined();
    expect(task!.ruleId).toBe('RULE-08-003');
  });

  it('GT-33: lịch sử dài (75 CK) chỉ 22 CK khóa rải rác xen kẽ → CLASSIFICATION_BLOCKED, không phải D, và Chặng 12 không tự phát hành dự báo', () => {
    const { engine, snapshot } = runTo6();
    const skuId = Object.keys(snapshot.states)[0];
    ...
    let s: StageSnapshot = engine.run(7, snapshot, DEFAULT_POLICY);
    for (let stage = 8; stage <= 12; stage++) s = engine.run(stage as StageNumber, s, DEFAULT_POLICY);
  ...
  it('GT-36: SKU-014 (mock catalog có sẵn — 6 chu kỳ khóa, baseDemand=0) → NO_POSITIVE_DEMAND_REVIEW, KHÔNG phải D (hồi quy: trước bản sửa sẽ là D)', () => {
    const engine = testEngine();
    let snapshot: StageSnapshot | null = null;
    for (let stage = 1; stage <= 8; stage++) snapshot = engine.run(stage as StageNumber, snapshot, DEFAULT_POLICY);
...
describe('RULE-12-001 — chuỗi học Chặng 12 không bỏ khoảng trống rồi nén chuỗi', () => {
  it('có một khoảng đứt quãng xa hơn 24 chu kỳ (ngoài cửa sổ XYZ) → phân loại vẫn CLASSIFIED, nhưng Chặng 12 vẫn ghi nhận lịch sử cũ hơn bị loại (FORECAST_INPUT_BLOCKED), không im lặng nén chuỗi', () => {
    const { engine, snapshot } = runTo6();
    const skuId = Object.keys(snapshot.states)[0];
    const cycles: CycleRecord[] = [];
    for (let i = 1; i <= 10; i++) cycles.push(makeCycle(i, true, 'LOCKED_OBSERVED'));
    cycles.push(makeCycle(11, false, 'BASELINE_UNRESOLVED')); // gap CÁCH cuối 29 chu kỳ — ngoài cửa sổ 24 của Chặng 8
    ...
    // Nhưng Chặng 12 vẫn phải biết CK1-10 KHÔNG được dùng chung với CK12-40 (khác 39 CK bị nén sai).
    const task = s.exceptions.find(item => item.skuId === skuId && item.code === 'FORECAST_INPUT_BLOCKED');
    expect(task).toBeDefined();
    expect(task!.ruleId).toBe('RULE-12-001');
    expect(task!.evidence).toContain('29/40');
  });

  it('GT-37 (dạng chặt): gap nằm ngay trong cửa sổ gần nhất → Chặng 8 CLASSIFICATION_BLOCKED và Chặng 12 chặn cứng luôn, không tự chuyển D, không nén chuỗi', () => {
    const { engine, snapshot } = runTo6();
    const skuId = Object.keys(snapshot.states)[0];
    const cycles: CycleRecord[] = [];
...
describe('RULE-08-002 §2.2 LỆNH CODEX — seriesQualityRatio chia cho ĐỘ DÀI CỬA SỔ, không chia cho toàn bộ lịch sử', () => {
