...
function runTo(stage: 7 | 8 | 9): StageSnapshot {
...
describe('RULE-07-001/002 — Chặng 7 ABC official/approval', () => {
  ...
  it('portfolioMode=FULL_PORTFOLIO → ABC được coi là chính thức', () => {
    ...
    for (let stage = 2; stage <= 7; stage++) snapshot = engine.run(stage as StageNumber, snapshot, DEFAULT_POLICY);
    const officials = Object.values(snapshot!.states).filter(state => state.classification.abcOfficial);
...
describe('RULE-08-001/002 — Chặng 8 phân loại D theo lý do', () => {
  ...
  it('D_SHORT_HISTORY chỉ đạt tới khi extractIsTruncated=false (biết chắc không bị cắt)', () => {
    ...
    for (let stage = 2; stage <= 8; stage++) snapshot = engine.run(stage as StageNumber, snapshot, DEFAULT_POLICY);
  ...
  it('audit Chặng 8 gắn RuleId RULE-08-001/002', () => {
    const snapshot = runTo(8);
    expect(snapshot.audit.some(line => line.includes('[RULE-08-001]'))).toBe(true);
    expect(snapshot.audit.some(line => line.includes('[RULE-08-002]'))).toBe(true);
  });
});

describe('RULE-09-001/002 — Chặng 9 chính sách', () => {
  it('SKU nhóm D/N-A → serviceLevel=null (không dùng 0), tạo task POLICY_UNRESOLVED', () => {
    ...
    expect(task!.ruleId).toBe('RULE-09-002');
  });

  it('audit Chặng 9 ghi rõ phiên chính sách (RULE-09-001) và giới hạn không hồi tố', () => {
    const snapshot = runTo(9);
    expect(snapshot.audit.some(line => line.includes('[RULE-09-001]') && line.includes(DEFAULT_POLICY.version))).toBe(true);
    expect(snapshot.audit.some(line => line.includes('[RULE-09-002]'))).toBe(true);
  });
});
