...
describe('stage-insights — panel trái đối ứng từng chặng', () => {
  ...
  it('bảng ABC/XYZ/ma trận phủ đúng toàn bộ danh mục', () => {
    const states6 = snapshots[7]!.states;
    ...
    const xyz = buildXyzBoard(snapshots[8]!.states);
    expect(xyz).toHaveLength(board.length);
    const matrix = buildPolicyMatrix(snapshots[9]!.states, 'SKU-001');
    const total = matrix.totalInMatrix + matrix.exceptions.count;
    expect(total).toBe(board.length);
  });

  it('các cột kết quả Chặng 7–8 khớp dữ liệu engine đã khóa', () => {
    const abc = buildAbcBoard(snapshots[7]!.states);
    ...
    for (const row of xyz) {
      const locked = snapshots[8]!.states[row.id].classification;
  ...
  it('mọi SKU đều dựng được insight ở mọi chặng 10–16 mà không lỗi', () => {
    for (const skuId of Object.keys(snapshots[16]!.states)) {
      expect(() => {
        buildSeasonalityAudit(snapshots[10]!.states[skuId]);
        buildTrendAudit(snapshots[11]!.states[skuId]);
        buildForecastAudit(snapshots[12]!.states[skuId]);
        buildPromoAudit(snapshots[13]!.states[skuId]);
        buildFinalForecastAudit(snapshots[14]!.states[skuId]);
        buildSupplyAudit(snapshots[15]!.states[skuId]);
        buildSafetyAudit(snapshots[16]!.states[skuId]);
      }, `SKU ${skuId}`).not.toThrow();
    }
  });

  it('diễn biến học Chặng 12 dựng lại khớp tuyệt đối kết quả engine đã khóa', () => {
  ...
  it('bảng mùa vụ Chặng 10 khớp kết luận engine cho SKU nhóm Y đủ chuỗi', () => {
    for (const state of Object.values(snapshots[10]!.states)) {
  ...
  it('hệ số K Chặng 13 dựng lại khớp promoFactor engine', () => {
    for (const state of Object.values(snapshots[13]!.states)) {
  ...
  it('Chặng 16 ưu tiên sai số backtest và quy đổi lead time theo độ dài chu kỳ', () => {
    const snapshots = runPipeline();
    const state = Object.values(snapshots[16]!.states).find(item => item.serviceLevel && item.forecast?.model !== 'PurchasePlan')!;
