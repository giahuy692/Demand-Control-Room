...
describe('RULE-06-003/RULE-05-006 — cửa sổ ABC là 24 vị trí gần nhất theo lịch, đếm CK khóa bất kể khoảng khuyết (§2.1 LỆNH CODEX)', () => {
  it('GT-32/34 (đã cập nhật theo §2.1): CK1-15 khóa, CK16-24 unresolved, CK25 khóa → cửa sổ 24 vị trí có 15 CK khóa rải rác quanh khoảng khuyết → VẪN đủ ngưỡng 6, ANNUALIZED_WITH_GAPS, không tạo ABC_INPUT_BLOCKED', () => {
    ...
    // Cửa sổ 24 vị trí gần nhất = CK2..CK25: 12 CK khóa (CK2-13) + CK14 + CK15 + CK25 = 15 CK khóa.
    expect(state.classification.abc).not.toBe('N/A');
    expect(state.classification.abcStatus).toBe('annualized');
    expect(state.classification.lockedCycles).toBe(15);
    expect(state.classification.annualizationFactor).toBeCloseTo(24 / 15, 5);
  ...
  it('không nén vị trí CK hay đổi thứ tự chuỗi: periodQuantity chỉ cộng đúng các CK khóa trong cửa sổ, bỏ qua CK chưa khóa', () => {
    const { engine, snapshot } = runTo5();
    const skuId = Object.keys(snapshot.states)[0];
    const cycles: CycleRecord[] = [];
    ...
    expect(state.classification.lockedCycles).toBe(12);
    expect(state.classification.periodQuantity).toBeCloseTo(6 * 10 + 6 * 20, 5);
    expect(state.classification.abcStatus).toBe('annualized');
  });
});
